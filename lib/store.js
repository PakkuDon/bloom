...
class Store {
  ...
  findOrganisations(query) {
    const matchingOrganisations = this.organisations.where((organisation) =>
      doesMatchQuery({ record: organisation, query })
    );

    return matchingOrganisations.map((organisation) => ({
      ...organisation,
      users: this.users.where(
        (user) => user.organization_id === organisation._id
      ),
      tickets: this.tickets.where(
        (ticket) => ticket.organization_id === organisation._id
      ),
    }));
  }
}
